...
char *read_file(char *buf, int size, FILE *stream)
{
    ...
    temp = fgets(buf,size,stream);
	/* Change any lines which are CRLF to LF */
	len = strlen(buf);	
	if (buf[len - 1] == '\n') {
		if(buf[len - 2] == '\r') {
			buf[len - 2] = '\n';
		...
		/* Handle the case where the CR is right at EOF */
		/* which should never be the case, but what the heck */
		buf[len - 1] = '\n';
	}
    log_printf( 85, "read file line: %s", temp );
    return temp;
}
...
int write_file(FILE *stream, const char *format, ...)
{
    ...
    /* Allow arbitrary sized strings and also debug printout */
    /* Code stolen from man vnsprintf */
	...
	result = vsnprintf (temp, size, format, vargs);
	va_end(vargs);
    ...
    */
    ...
    result = fwrite(temp, 1, result, stream);

    log_printf( 85, "write file line: %s", temp );
	free(temp); // lets not leak memory, eh?
    return result;
}
