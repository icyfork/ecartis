...
void make_list_aliases(const char *listname, const char *siteconfig, const char *hostname)
{
   ...
   if (LMAPI->get_bool("newlist-procmail")) {
     ...
     vhpref = LMAPI->get_string("vhost-alias-prefix");
     ...
     printf("%s%s-request: \"|%s/%s %s-r %s\"\n",
       vhpref ? vhpref : "",
       listname, LMAPI->get_string("listserver-bin-dir"),
       SERVICE_NAME_LC, siteconfig, listname);

     printf("%s%s-repost: \"|%s/%s %s-a %s\"\n",
       vhpref ? vhpref : "",
       listname, LMAPI->get_string("listserver-bin-dir"),
       SERVICE_NAME_LC, siteconfig, listname);

     printf("%s%s-admins: \"|%s/%s %s-admins %s\"\n",
       vhpref ? vhpref : "",
       listname, LMAPI->get_string("listserver-bin-dir"),
       SERVICE_NAME_LC, siteconfig, listname);

     printf("%s%s-moderators: \"|%s/%s %s-moderators %s\"\n",
       vhpref ? vhpref : "",
       listname, LMAPI->get_string("listserver-bin-dir"),
       SERVICE_NAME_LC, siteconfig, listname);

     printf("%s%s-bounce: \"|%s/%s %s-bounce %s\"\n",
       vhpref ? vhpref : "",
       listname, LMAPI->get_string("listserver-bin-dir"),
       SERVICE_NAME_LC, siteconfig, listname);
   } else {
...
CMDARG_HANDLER(cmdarg_newlist)
{
   if(!argv[0]) {
	   ...
	   if (listname[0] == '/' || strstr(listname, "../") != NULL) {
		   fprintf(stderr, "List '%s' could not be created because of invalid characters in the list name.", argv[0]);
		   return CMDARG_ERR;
	   }
