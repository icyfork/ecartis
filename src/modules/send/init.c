...
void send_load(struct LPMAPI *api)
{
	...
	LMAPI->register_var("overquoting-include-queue", "no", "Quoting",
						"If a message is rejected for overquoting, should the original message be included in the reply which is sent back?",
						"overquoting-include-queue = no", VAR_BOOL, VAR_ALL);
