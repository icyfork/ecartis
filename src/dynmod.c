...
typedef void (*dl_fptr)(void);
typedef void (*dl_fptr_LPMAPI)(struct LPMAPI * api);
typedef void (*dl_fptr_int_int)(int a, int b);
#endif
#ifdef OBSDMOD
...
int load_a_module(char *pathname, char *modname)
{
    ...
    if(S_ISREG(s.st_mode) && ((s.st_mode & S_IRUSR) != 0)) {
        ...
        loadFunc = (dl_fptr_LPMAPI) dlsym(dlHandle, funcname);
        if(loadFunc) {
            (*loadFunc)(&API);
...
int init_all_modules()
{
    ...
    while(tempref) {
        ...
        initFunc = (dl_fptr) dlsym(tempref->handle, funcname);
        if(initFunc)
            (*initFunc)();
...
int unload_all_modules()
{
    ...
    while(tempref) {
        ...
        unloadFunc = (dl_fptr) dlsym(tempref->handle, funcname);
        if(unloadFunc)
            (*unloadFunc)();
        tempref = tempref->next;
    }
...
int upgrade_all_modules(int prev, int cur)
{
    struct listserver_modref *tempref;
    char funcname[MAX_FNAME];
    dl_fptr_int_int upgradeFunc;

    tempref = mod_handles;
    while(tempref) {
        buffer_printf(funcname, sizeof(funcname) - 1, UPGRADE_FUNCNAME_PATTERN, tempref->name);
        upgradeFunc = (dl_fptr_int_int) dlsym(tempref->handle, funcname);
        if(upgradeFunc)
